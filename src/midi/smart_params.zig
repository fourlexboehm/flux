//! Ranked CLAP parameter pages for hardware knobs (Axiom-class surfaces).
//!
//! Chrome owns the table (`ui/state.ControllerState`); this module rebuilds and
//! pages it when the device-chain target plugin changes.
...
const ControllerSmartParam = chrome.ControllerSmartParam;
const controller_smart_slots = chrome.controller_smart_slots;
...
pub fn pageCount(state: *const chrome.State) usize {
...
pub fn setPageDelta(state: *chrome.State, delta: i32) void {
...
pub fn slotForKnob(state: *const chrome.State, knob_index: usize) ?ControllerSmartParam {
...
/// Rebuild the ranked param list when the target plugin/slot changes.
/// `plugin_token` is `@intFromPtr(plugin)` or 0 when none.
pub fn rebuildIfNeeded(
    state: *chrome.State,
    allocator: std.mem.Allocator,
    plugin: ?*const clap.Plugin,
    target_track: usize,
    target_kind: chrome.DeviceTargetKind,
    target_fx: usize,
) void {
    const token: usize = if (plugin) |p| @intFromPtr(p) else 0;

    if (state.controller.smart_target_token == token and
    ...
    state.controller.smart_target_token = token;
...
fn buildLabel(info: *const clap.ext.params.Info) [96]u8 {
    ...
    const module = std.mem.sliceTo(&info.module, 0);
    var idx: usize = 0;
