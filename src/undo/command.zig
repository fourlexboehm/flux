...
/// Replace entire note list for a clip (bulk piano-roll tools).
pub const NotesReplaceCmd = struct {
    track: usize,
    scene: usize,
    old_notes: []const Note,
    new_notes: []const Note,
    old_timing: piano_roll_types.ClipTiming,
    new_timing: piano_roll_types.ClipTiming,
};
...
pub const Command = union(CommandKind) {
    ...
    pub fn deinit(self: *Command, allocator: std.mem.Allocator) void {
        switch (self.*) {
            ...
            .notes_replace => |*cmd| {
                if (cmd.old_notes.len > 0) allocator.free(cmd.old_notes);
                if (cmd.new_notes.len > 0) allocator.free(cmd.new_notes);
            },
            .track_delete => |*cmd| {
                if (cmd.notes.len > 0) {
