//! Legacy zgui host (reference only — not built or installed).
//!
//! Port behavior from here and `src/ui_zgui/` into `src/ui/` + `src/main.zig`.
//! Need a runnable zgui app? Check out a prior commit in a git worktree.
//!
//! Unit tests use `src/tests.zig` as the module root (see build.zig).
...
const ui_draw = @import("ui_zgui/draw.zig");
const ui_filters = @import("ui_zgui/panels/filters.zig");
const ui_keyboard = @import("ui_zgui/input/keyboard_midi.zig");
const ui_recording = @import("ui_zgui/recording.zig");
