...
test "wantsStretch defaults and overrides" {
    try std.testing.expect(wantsStretch(null));
    try std.testing.expect(wantsStretch("stretch"));
    try std.testing.expect(wantsStretch("STRETCH"));
    try std.testing.expect(!wantsStretch("varispeed"));
    try std.testing.expect(!wantsStretch("repitch"));
}
...
test "offline bake: inject asset and stretch to musical length" {
    ...
    var clip = AudioClip.init(allocator);
    ...
    try ensureBaked(&clip, &store, 120, host_sr);
    ...
    try ensureBaked(&clip, &store, 140, host_sr);
    try std.testing.expect(clip.hasBaked());
