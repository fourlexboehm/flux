...
test "media_layout path safety and sanitize" {
    ...
    try std.testing.expect(isSafeRelativePath("samples/kick.wav"));
    try std.testing.expect(isSafeRelativePath("recordings/Track1-001.wav"));
    try std.testing.expect(!isSafeRelativePath("../etc/passwd"));
    try std.testing.expect(!isSafeRelativePath("/abs/path.wav"));
    try std.testing.expect(isMediaSubdirPath("samples/a.wav"));
    try std.testing.expect(isMediaSubdirPath("recordings/b.wav"));
    try std.testing.expect(!isMediaSubdirPath("audio/c.wav"));

    const safe = try sanitizeBaseName(allocator, "foo/../../kick drum!.wav");
    defer allocator.free(safe);
