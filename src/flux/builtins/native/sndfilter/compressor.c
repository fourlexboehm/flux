...
void sf_compressor_process(sf_compressor_state_st *state, int size, sf_sample_st *input,
	...
	int chunks = (size + samplesperchunk - 1) / samplesperchunk;
	...
	for (int ch = 0; ch < chunks; ch++){
		...
		// Process a final partial chunk too. Upstream assumes all host blocks are
		// divisible by 32; CLAP permits arbitrary frame counts.
		int chunksamples = size - samplepos;
		if (chunksamples > samplesperchunk)
			chunksamples = samplesperchunk;
		for (int chi = 0; chi < chunksamples; chi++, samplepos++,
